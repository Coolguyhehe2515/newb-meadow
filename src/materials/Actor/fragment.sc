...
void main() {
  ...
  #ifdef NL_ENTITY_LIGHT_TINT
    float torchFactor = light.a;
    float lum = dot(albedo.rgb, vec3(0.2126,0.7152,0.0722));
    albedo.rgb = mix(vec3_splat(lum), albedo.rgb, mix(NL_ENTITY_DESAT_DARK, 1.0, torchFactor));
    albedo.rgb *= mix(NL_ENTITY_TINT_DARK, NL_ENTITY_TINT_LIT, torchFactor);
  #endif
