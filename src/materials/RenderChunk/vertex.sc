...
void main() {
  ...
  fogColor.a = nlRenderFogFade(relativeDist, FogColor.rgb, FogAndDistanceControl.xy);
    ...
    fogColor.a = mix(fogColor.a, 1.0, min(NL_GODRAY*nlRenderGodRayIntensity(cPos, worldPos, t, uv1, relativeDist, env), 1.0));
  #endif
  ...
  if (env.underwater) {
    nlUnderwaterLighting(light, pos.xyz, lit, uv1, tiledCpos, cPos, t, skycol.horizon);
      ...
      if (env.sunDir.y > 0.0) {
        float uwRay = nlRenderUnderwaterGodRay(cPos, worldPos, t, uv1, env.sunDir);
        fogColor.rgb += uwRay*NL_UNDERWATER_BRIGHTNESS;
        fogColor.a = mix(fogColor.a, 1.0, min(NL_UNDERWATER_GODRAY*uwRay, 1.0));
      }
    #endif
  }
