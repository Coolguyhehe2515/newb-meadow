...
void main() {
  vec4 color = v_color0;
    ...
    cloudPos.xz += CameraPosition.xz;
      ...
      color = renderCloudsLayered(vDir, v_color2.w, v_color1.w);

      #ifdef NL_AURORA
        vec2 auroraP = vDir.xz / (0.015 + 0.035*abs(vDir.y));
        auroraP.xy *= 34.7;
        color += renderAurora(auroraP.xyy, v_color2.w, v_color1.w, v_fogColor)*(1.0-0.95*color.a);
      #endif
