...
vec3 nlRenderBlackHole(vec3 skyBg, vec3 viewDir, float t, sampler2D noiseTex) {
  vec3 bhDir = normalize(NL_BLACKHOLE_DIR);
  vec3 right = normalize(cross(vec3(0.0,0.0,1.0), bhDir));
  vec3 up = cross(bhDir, right);

  float z = dot(viewDir, bhDir);
  if (z < 0.3) {
    return skyBg;
  }

  float x = dot(viewDir, right)/z;
  float y = dot(viewDir, up)/z;

  float r = length(vec2(x, y));
  float phi = atan2(y, x);

  float size = NL_BLACKHOLE_SIZE;
  float horizon = 1.0 - smoothstep(size*0.92, size, r);

  float spiralPhi = phi + 2.2/(r/size + 0.15) - t*NL_BLACKHOLE_SPEED*4.0;
  vec2 noiseUV = vec2(0.15*spiralPhi + 0.06*r/size, 0.3*r/size + 0.02*t);
  float n1 = texture2D(noiseTex, noiseUV).r;
  float bands = 0.5 + 0.5*sin(spiralPhi*3.0 + 6.0*r/size);
  bands = bands*bands*bands;
  float arms = 0.35 + 0.4*n1 + 0.5*bands;

  float diskFalloff = smoothstep(size*0.95, size*1.1, r) * (1.0-smoothstep(size*2.2, size*4.5, r));
  float ring = smoothstep(size*1.05, size*0.98, r) * (1.0-horizon);

  vec3 glow = NL_BLACKHOLE_GLOW_COL*diskFalloff*arms;
  glow += NL_BLACKHOLE_DISK_COL*ring*arms;

  vec3 result = mix(skyBg, vec3(0.0), horizon);
  result += glow*(1.0-horizon);
