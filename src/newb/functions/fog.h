...
// underwater godray - same sine-streak trick as above, but reoriented to the
// real sun direction (so the shaft angle actually follows time of day) and
// gated to underwater + sun above horizon. deliberately has no relativeDist
// falloff - that falloff suppresses the effect near the camera, which is
// exactly where underwater visibility exists, making it invisible in practice.
float nlRenderUnderwaterGodRay(vec3 cPos, vec3 worldPos, float t, vec2 uv1, vec3 sunDir) {
  vec3 offset = cPos - 16.0*fract(worldPos*0.0625);
  offset = abs(2.0*fract(offset*0.0625)-1.0);
  offset = offset*offset*(3.0-2.0*offset);

  // build a basis aligned to the real sun direction so the streak
  // orientation follows the sun instead of a fixed world axis
